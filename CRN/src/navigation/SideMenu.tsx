...
export default function SideMenu() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      Animated.parallel([
        Animated.timing(slideAnim, { toValue: -PANEL_WIDTH, duration: ANIM_DURATION, useNativeDriver: true }),
          ...
          >
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, paddingHorizontal: 4 }}>
              ...
              <Button
                ...
                status="basic"
                size="medium"
                accessoryLeft={NavIcon(isDark ? 'sun-outline' : 'moon-outline')}
                onPress={() => setMode(isDark ? 'light' : 'dark')}
                style={{ paddingHorizontal: 0, marginRight: -4 }}
              />
            </View>
            ...
            {NAV_ITEMS.map(({ label, route, icon }) => (
              <Button
                ...
                onPress={() => goTo(route)}
                style={{ justifyContent: 'flex-start', marginVertical: 2, paddingHorizontal: 0 }}
              >
                {label}
              </Button>
            ))}
